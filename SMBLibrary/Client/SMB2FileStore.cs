...
namespace SMBLibrary.Client
{
    public class SMB2FileStore : ISMBFileStore
    {
        ...
        public async Task<NTStatus> CloseFile(object handle)
        {
            ...
            var response = await TrySendCommand<SMB2Response>(request);
            if (response != null)
            {
        ...
        public async Task<NTResult<IMemoryOwner<byte>>> ReadFile(object handle, long offset, int maxCount)
        {
            ...
            if (response != null)
            {
                ...
                {
                    return new ArrayMemoryOwner(readResponse.Data, 0, (int)readResponse.DataLength, readResponse.Pool, m_client.errorLog);
                }
                return response.Header.Status;
            }
        ...
        private Task<TResponse> TrySendCommand<TResponse>(SMB2Request request) where TResponse : SMB2Response
        {
